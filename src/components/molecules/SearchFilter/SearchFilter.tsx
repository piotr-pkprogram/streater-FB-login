...
import { FiltersTypes, SortModes } from 'hooks/useFoodtrucks';
...
const SearchFilter = forwardRef(
  ({ foodtrucks, setFilter, setVisible, SortMode, setSortMode }: Props, ref) => {
            ...
            <div>
              ...
              <ul className="ml-3">
                {foodtrucks.map((foodtruck) => {
                  foodtruck.menu.kitchenType.map(({ name }) => {
                    if (name !== 'string' && name)
                      return (
                        <li>
                          <Checkbox label={name} opt={name.toLowerCase()} />
                        </li>
                      );
                  });
                })}
